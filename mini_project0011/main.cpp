...
int main(int argc, char* argv[]) {
	...
	std::cout << "이동 : W A S D"<<endl;
	std::cout << "number 1: 총기 몸체 분리 "<<endl;
	std::cout << "number 2: 발사 "<<endl;
	std::cout << "number 3: 개머리판 위치 조정 "<<endl;
	std::cout << "number 4: 탄알집 분리 "<<endl;
	std::cout << "number 5: 반투명모드 "<<endl;
	std::cout << "number r: 회전 "<<endl;
	std::cout << "number I/o: 줌인 줌아웃 "<<endl;
	std::cout << "number [,]: 민감도 조절 "<<endl;
	std::cout << "마우스로 selection하고 스크롤 : 총기 이동 "<<endl;
	std::cout << "마우스 좌클릭 : Game Mode"<<endl;
	std::cout << "Game Mode R : 장전"<<endl;
	std::cout << "Game Mode M : 배경음악 on"<<endl;
	std::cout << "Game Mode n : 배경음악 off"<<endl;
...
void mappingTexture(void) {
	...
	SDL_Surface* img0 = IMG_Load("Texture/forward1.bmp");	// 오른쪽
	SDL_Surface* img1 = IMG_Load("Texture/forward1.bmp");	// 왼쪽
	SDL_Surface* img2 = IMG_Load("Texture/forward1.bmp");	// 위
	SDL_Surface* img3 = IMG_Load("Texture/ground.bmp");	// 아래
	SDL_Surface* img4 = IMG_Load("Texture/forward1.bmp");	// 앞
	SDL_Surface* img5 = IMG_Load("Texture/forward1.bmp");	// 뒤

	glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X, 0, GL_RGBA, img0->w, img0->h, 0, GL_RGB, GL_UNSIGNED_BYTE, img0->pixels);	// 오른쪽
	glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_X, 0, GL_RGBA, img1->w, img1->h, 0, GL_RGB, GL_UNSIGNED_BYTE, img1->pixels);	// 왼쪽
	glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Y, 0, GL_RGBA, img2->w, img2->h, 0, GL_RGB, GL_UNSIGNED_BYTE, img2->pixels);	// 위
	glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Y, 0, GL_RGBA, img3->w, img3->h, 0, GL_RGB, GL_UNSIGNED_BYTE, img3->pixels);	// 아래
	glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Z, 0, GL_RGBA, img4->w, img4->h, 0, GL_RGB, GL_UNSIGNED_BYTE, img4->pixels);	// 앞
	glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Z, 0, GL_RGBA, img5->w, img5->h, 0, GL_RGB, GL_UNSIGNED_BYTE, img5->pixels);	// 뒤
...
void draw(void) {
	...
	glRotatef(-T_Rotate, 1, 0, 0); // 표적이 hits시 뒤로 90도 내려갔다가 올라옴
...
void mouse(int button, int state, int x, int y) {
	...
	if (game_mode) {
		...
		if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
			if (bullet_count > 0) {
				Mix_PlayChannel(-1, gGun, 0);	// 총성 출력
...
/* 코드 참고 : http://www.opengl-tutorial.org/kr/beginners-tutorials/tutorial-7-model-loading/ */
void obj_file_load(char* fname) {
	FILE* fp;
	float x;
	float y;
	...
	while (!(feof(fp))) {
		char lineHeader[128];
		fscanf(fp, "%s", lineHeader);	// 첫번째 라인의 단어를 읽습니다.
...
void draw_obj() {
	...
	for (int object = 0; object < object_number; object++) {
		...
		if (game_mode) {
			...
			glTranslatef(viewX - T_Trans_X-5 , viewY, viewZ - T_Trans_Z+3);	// 1인칭 총 시점 조절
			glRotatef(mouse_rotationY, 1, 0, 0);
			glRotatef(mouse_rotationX, 0, 1, 0);
			glTranslatef(-viewX + T_Trans_X, -viewY, -viewZ + T_Trans_Z);	// 총 위치
		}
		else {
			// 개머리판
			...
			if (blending) {
				...
				if (object == 0 || object == 1 || object == 2 || object == 8 || object == 5 || object == 6 || object == 7 || object == 16 || object == 17 || object ==  18) {
					...
					glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);	// 반투명 출력
